...
combined_result <- readRDS("data/processed_data/04_analysis_ready/cross_year_combined_data.rds")
stopifnot(
  "INC_FAM_ANALYSIS_ELIGIBLE" %in% names(combined_result),
  combined_result$INC_FAM_ANALYSIS_ELIGIBLE[combined_result$DATA_Y == 2014L] %in% FALSE
)
...
recoded_values <- read.csv(
  "output/summary_statistics/income_expenditure_recoded/income_expenditure_recoded_values.csv",
  check.names = FALSE
)
stopifnot(all(c(
  "INC_FAM_TOTAL_COMMON3_INCOME",
  "EXP_TOTAL_COMMON5_EXPENDITURE"
) %in% recoded_values$variable))
stopifnot(all(
