...
combined_data <- combined_data %>%
  ...
  mutate(
    INC_FAM_ANALYSIS_ELIGIBLE = case_when(
      DATA_Y == 2014L ~ ELIG_INC_FAM_COMPONENTS %in% TRUE,
      TRUE ~ TRUE
    ),
    INC_FAM_ANALYSIS_ELIGIBILITY_REASON = case_when(
      DATA_Y == 2014L & ELIG_INC_FAM_COMPONENTS %in% TRUE ~ "I1有其他家人固定收入；I3-I8有詢問",
      DATA_Y == 2014L & ELIG_INC_FAM_COMPONENTS %in% FALSE ~ "I1沒有其他家人固定收入；問卷跳答I9",
      DATA_Y == 2014L ~ "I1無法判定；收入分析需複核",
      TRUE ~ "該年度無I1跳答排除規則"
    )
  )
