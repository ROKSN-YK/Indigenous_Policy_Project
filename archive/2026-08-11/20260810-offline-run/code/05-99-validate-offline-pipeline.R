...
income_mapping <- read_csv("output/checks/check_income_variable_mapping.csv", show_col_types = FALSE)
mapping_2010_other <- income_mapping %>%
  filter(data_year == 2010L, integrated_var == "INC_FAM_OTHER")
add_validation(
  "income_2010_family_other_override",
  nrow(mapping_2010_other) == 1L &&
    mapping_2010_other$expected_raw_var[[1]] == "I6" &&
    mapping_2010_other$resolved_dataset_var[[1]] == "i7" &&
    mapping_2010_other$status[[1]] == "ok_override",
  ifelse(
    nrow(mapping_2010_other) == 1L,
    paste(mapping_2010_other$expected_raw_var, mapping_2010_other$resolved_dataset_var, mapping_2010_other$status, sep = ":"),
    "missing"
  ),
  "I6:i7:ok_override"
)

unexpected_spec_failures <- income_mapping %>%
  filter(!is.na(expected_raw_var), expected_raw_var != "", !status %in% c("ok", "ok_override"))
add_validation(
  "specified_income_questions_resolve",
  nrow(unexpected_spec_failures) == 0L,
  nrow(unexpected_spec_failures),
  "0"
)

station_combined_path <- "data/processed_data/04_analysis_ready/cross_year_combined_with_care_station.rds"
station_panel_check_path <- "output/checks/check_care_station_panel_build.csv"
station_merge_check_path <- "output/checks/check_care_station_merge_coverage.csv"
station_files_present <- all(file.exists(c(station_combined_path, station_panel_check_path, station_merge_check_path)))
add_validation(
  "care_station_outputs_present",
  station_files_present,
  station_files_present,
  "TRUE"
)
if (station_files_present) {
  station_combined <- readRDS(station_combined_path) %>% as_tibble()
  station_panel_check <- read_csv(station_panel_check_path, show_col_types = FALSE)
  add_validation(
    "care_station_merge_preserves_respondents",
    nrow(station_combined) == nrow(combined) &&
      !anyDuplicated(station_combined[c("ID", "DATA_Y")]),
    paste(nrow(combined), nrow(station_combined), sep = ":"),
    "same row count and unique ID-year"
  )
  add_validation(
    "care_station_panel_snapshot_reconciles",
    nrow(station_panel_check) == 1L &&
      station_panel_check$station_n[[1]] == 525L &&
      station_panel_check$town_n[[1]] == 122L &&
      isTRUE(station_panel_check$has_110_panel[[1]]),
    paste(
      station_panel_check$station_n[[1]],
      station_panel_check$town_n[[1]],
      station_panel_check$has_110_panel[[1]],
      sep = ":"
    ),
    "525:122:TRUE"
  )
...
add_validation(
  ...
  setequal(
    sort(unique(as.integer(indicator_directions$data_year))),
    c(2010L, 2014L, 2017L, 2021L)
  ) &&
...
income_2014 <- numeric_income_expenditure %>%
  filter(
    ...
    survey_year == 2014L,
    str_detect(variable, "^INC_FAM_.*_INCOME$")
  )
add_validation(
...
income_2014_eligibility <- combined %>%
  filter(DATA_Y == 2014L) %>%
  count(INC_FAM_ANALYSIS_ELIGIBLE, INC_FAM_ANALYSIS_ELIGIBILITY_REASON, name = "n")
add_validation(
  "family_income_2014_eligible_records_included",
  nrow(income_2014_eligibility) >= 2L &&
    sum(income_2014_eligibility$n[income_2014_eligibility$INC_FAM_ANALYSIS_ELIGIBLE %in% TRUE]) == 3345L &&
    sum(income_2014_eligibility$n[income_2014_eligibility$INC_FAM_ANALYSIS_ELIGIBLE %in% FALSE]) == 1869L,
  paste(
    income_2014_eligibility$INC_FAM_ANALYSIS_ELIGIBLE,
    income_2014_eligibility$n,
    sep = ":",
    collapse = ";"
  ),
  "TRUE:3345;FALSE:1869",
  "I1有的3345筆納入；I1沒有且跳答I9的1869筆保留為結構性不適用，不補零。"
)
