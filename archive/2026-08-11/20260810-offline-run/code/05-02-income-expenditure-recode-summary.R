...
append_derived_totals <- function(recoded_data) {
  ...
  reported_names <- c(
    ...
    EXP_TOTAL_REPORTED_EXPENDITURE = "EXP_TOTAL_REPORTED_EXPENDITURE"
  )
  derived_names <- c(
    INC_FAM_TOTAL_INCOME = "INC_FAM_TOTAL_DERIVED_INCOME",
    EXP_TOTAL_COMPONENTS = "EXP_TOTAL_DERIVED_EXPENDITURE"
  )
  complete_names <- c(
    INC_FAM_TOTAL_INCOME = "INC_FAM_TOTAL_DERIVED_COMPLETE_INCOME",
    EXP_TOTAL_COMPONENTS = "EXP_TOTAL_DERIVED_COMPLETE_EXPENDITURE"
  )
  ...
  derived <- map_dfr(sort(unique(recoded_data$DATA_Y)), function(one_year) {
    imap_dfr(total_specs, function(components, total_var) {
      if (total_var == "EXP_TOTAL_COMPONENTS") {
...
append_fixed_common_totals <- function(recoded_data) {
  fixed_specs <- list(
    INC_FAM_TOTAL_COMMON3_INCOME = c(
      "INC_FAM_WORK_INCOME", "INC_FAM_GOV_INCOME", "INC_FAM_OTHER_INCOME"
    ),
    EXP_TOTAL_COMMON5_EXPENDITURE = c(
      "EXP_ALCOHOL_EXPENDITURE", "EXP_FOOD_EXPENDITURE",
      "EXP_MEDICAL_EXPENDITURE", "EXP_TRANSPORT_COMM_EXPENDITURE",
      "EXP_TRAVEL_EXPENDITURE"
    )
  )

  fixed_totals <- imap_dfr(fixed_specs, function(components, total_var) {
    recoded_data %>%
      filter(variable %in% components) %>%
      group_by(sample_definition, DATA_Y, ID) %>%
      summarise(
        component_expected_n = length(components),
        component_valid_n = n_distinct(variable[!is.na(recoded_midpoint)]),
        component_complete = component_valid_n == component_expected_n,
  ...
  bind_rows(recoded_data, fixed_totals)
}
  ...
  recoded_data %>%
    ...
    mutate(
      structurally_ineligible = case_when(
        ...
        DATA_Y == 2014L & str_detect(variable, "^INC_FAM_") ~ ELIG_INC_FAM_COMPONENTS %in% FALSE,
...
target_vars <- names(combined_data)[str_detect(names(combined_data), "^(INC_|EXP_)")] %>%
  ...
  union(intersect("RENT", names(combined_data)))
...
recoded_dataset <- recoded_base_dataset %>%
  append_derived_totals() %>%
  append_fixed_common_totals()
