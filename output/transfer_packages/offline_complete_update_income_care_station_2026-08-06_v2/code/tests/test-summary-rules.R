  ...
  if (
    is.call(expr) &&
      ...
      as.character(expr[[2]]) %in% c(
        "classify_two_stage_values", "apply_two_stage_indicator_override",
        "classify_unlabelled_numeric"
      )
  ) {
    eval(expr, envir = globalenv())
  }
...
expect_equal(parse_money_range("1,000-2,999元\uF020")$midpoint, 1999.5, "2014 rent private-use suffix")
...
override_cases <- apply_two_stage_indicator_override(
  classify_two_stage_values(c("�", "有"), c("1,000-1,999元", "1,000-1,999元")),
  c(2L, 1L),
  tibble(no_code = 2L, amount_conflict_action = "indicator_no_wins")
)
expect_equal(override_cases$explicit_no, c(TRUE, FALSE), "2017 corrupted-label code override")
expect_equal(override_cases$zero_from_indicator, c(TRUE, FALSE), "2017 indicator-no wins over stored amount")
expect_equal(override_cases$conflict, c(TRUE, FALSE), "2017 conflict remains auditable")

unlabelled_cases <- classify_unlabelled_numeric(
  underlying_numeric = c(1, 3, 4, 100000),
  labelled_codes = c(2, 5, 6),
  known_option_codes = 1:14,
  mapped = c(TRUE, TRUE, TRUE, TRUE)
)
expect_equal(unlabelled_cases, c(FALSE, FALSE, FALSE, TRUE), "known codes are not exact currency amounts")
